...
const HOME_POSITIONS = [0, 8, 16, 24];
...
let roomId = null, unsub = null, state = null, acted = false, actionPending = false;
let aiBusyKey = null;
...
$('buildBtn').onclick = openLandChoice;
$('endBtn').onclick = endTurn;
$('soundBtn').onclick = toggleSound;
$('rulesBtn').onclick = () => showModal('<h2>遊戲規則</h2><p>玩家從各自的總部出發。擲骰移動後，若停在無店鋪土地，可選擇建立店面或抽業務員卡；若停在顧客區/營運計畫區則抽對應卡。經過或回到自己的總部領 3000 元，現金達 30000 元獲勝。</p>');
...
function makePlayer(id,name,idx,isAI){ const homeIndex = HOME_POSITIONS[idx % 4] ?? 0; return {id,name,avatar:AVATARS[idx]||'🛒',color:COLORS[idx]||'#999',money:START_MONEY,pos:homeIndex,homeIndex,isAI,freeBuild:0}; }
...
async function takeTurn(pid,dice,autoEnd){
 const s = structuredClone(state); const p = s.players[pid]; const old = p.pos; const home = p.homeIndex ?? 0;
 p.pos = (p.pos + dice) % 32;
 if(passesHome(old, dice, home)){ p.money += PASS_HQ_BONUS; s.logs = [`${p.name} 經過/回到自己的總部，領 ${PASS_HQ_BONUS}`, ...(s.logs||[])]; }
 s.logs = [`${p.name} 擲出 ${dice}，移動到 ${board[p.pos].name}`, ...(s.logs||[])];
 const needsChoice = resolveCell(s,p);
 ...
 if(pid === myId) actionPending = !!needsChoice;
 if(needsChoice && pid === myId) setTimeout(openLandChoice, 350);
 if(autoEnd && !s.winner && !needsChoice) setTimeout(() => advanceTurnFromSnapshot(), 700);
}
function passesHome(oldPos, dice, homeIndex){
 for(let step=1; step<=dice; step++) if((oldPos + step) % 32 === homeIndex) return true;
 return false;
}
function resolveCell(s,p){
 const cell = board[p.pos];
 if(cell.type==='customer') { customerEvent(s,p); return false; }
 if(cell.type==='strategy') { strategyEvent(s,p); return false; }
 ...
 if(cell.type==='land' && !s.buildings[p.pos]) return true;
 return false;
}
...
function openLandChoice(){
 if(!isMyTurn()) return;
 const p=state.players[myId], cell=board[p.pos], b=state.buildings?.[p.pos];
 if(cell.type!=='land') return alert('這格不是土地格');
 if(b && b.owner!==myId) return alert('這裡是別人的店鋪');
 if(b && b.owner===myId) return buildHere();
 showModal(`<h2>你停在「${cell.name}」</h2><p>請選擇這回合動作：</p><div class="choice-grid"><button id="chooseBuild">🏪 建立店面</button><button id="chooseSales">🎴 抽業務員卡</button></div>`);
 setTimeout(()=>{
   const build=$('chooseBuild'), sales=$('chooseSales');
   if(build) build.onclick=async()=>{ $('modal').close(); await buildHere(); actionPending=false; };
   if(sales) sales.onclick=async()=>{ $('modal').close(); await drawSalespersonCard(myId); actionPending=false; };
 },0);
}
...
async function drawSalespersonCard(pid){
 const s=structuredClone(state), p=s.players[pid];
 const cards=[
   {name:'熱情銷售員', text:'立即獲得 1000 元', money:1000},
   {name:'明星店員', text:'立即獲得 1500 元', money:1500},
   {name:'新人實習生', text:'獲得 500 元', money:500},
   {name:'巡店支援', text:'下次建店免費', freeBuild:1},
   {name:'銷售特訓', text:'獲得 800 元', money:800}
 ];
 const c=cards[Math.floor(Math.random()*cards.length)];
 if(c.money) p.money+=c.money;
 if(c.freeBuild) p.freeBuild=(p.freeBuild||0)+c.freeBuild;
 s.logs=[`🎴 ${p.name} 抽到業務員卡「${c.name}」：${c.text}`,...(s.logs||[])];
 if(p.money>=WIN_MONEY){s.winner=pid;s.logs=[`🏆 ${p.name} 達成 ${WIN_MONEY} 元，獲勝！`,...s.logs];}
 await updateDoc(roomRef(), {players:s.players, logs:s.logs.slice(0,40), winner:s.winner||null});
}
...
async function endTurn(){ if(!state || (currentId()!==myId && !state.players[currentId()]?.isAI)) return; acted=false; actionPending=false; await advanceTurnFromSnapshot(); }
async function advanceTurnFromSnapshot(){
 const snap = await getDoc(roomRef()); if(!snap.exists()) return;
 const s = snap.data(); if(s.winner) return;
 await updateDoc(roomRef(), {currentTurn:((s.currentTurn||0)+1)%(s.turnOrder?.length||1)});
}
async function aiPlay(){
 if(!isAITurn()) return;
 const key = `${roomId}_${state.currentTurn}_${currentId()}_${state.players[currentId()]?.pos}`;
 if(aiBusyKey === key) return;
 aiBusyKey = key;
 const id=currentId();
 await takeTurn(id, Math.floor(Math.random()*6)+1, false);
 await new Promise(r=>setTimeout(r,650));
 const snap = await getDoc(roomRef()); if(!snap.exists()) { aiBusyKey=null; return; }
 const s=snap.data(); if(s.winner || s.turnOrder[s.currentTurn] !== id){ aiBusyKey=null; return; }
 const p=s.players[id], cell=board[p.pos];
 if(cell.type==='land' && !s.buildings[p.pos]){
   if(p.money>BUILD_COST+1000 && Math.random()<0.65){ s.buildings[p.pos]={owner:id,level:'shop'}; p.money-=BUILD_COST; s.logs=[`${p.name} 選擇建立店面，花費 ${BUILD_COST}`,...(s.logs||[])]; }
   else { const bonus=[500,800,1000,1500][Math.floor(Math.random()*4)]; p.money+=bonus; s.logs=[`🎴 ${p.name} 選擇抽業務員卡，獲得 ${bonus}`,...(s.logs||[])]; }
   if(p.money>=WIN_MONEY){s.winner=id;s.logs=[`🏆 ${p.name} 達成 ${WIN_MONEY} 元，獲勝！`,...s.logs];}
   await updateDoc(roomRef(), {players:s.players, buildings:s.buildings, logs:s.logs.slice(0,40), winner:s.winner||null});
 }
 await new Promise(r=>setTimeout(r,700));
 await advanceTurnFromSnapshot();
 aiBusyKey=null;
}

function render(){ if(!state) return; renderPlayers(); renderBoard(); const cur=state.players[currentId()]; $('turnText').textContent = state.winner ? `🏆 ${state.players[state.winner].name} 獲勝！` : state.started ? `目前回合：${cur?.avatar||''} ${cur?.name||''}` : '等待房主開始遊戲'; $('startBtn').style.display = state.started ? 'none':'block'; $('rollBtn').disabled = !isMyTurn() || acted; const myCell = state.players?.[myId] ? board[state.players[myId].pos] : null;
 const canActOnLand = isMyTurn() && acted && myCell?.type === 'land';
 $('buildBtn').textContent = state.buildings?.[state.players?.[myId]?.pos]?.owner === myId ? '升級商場' : '建店 / 抽卡';
 $('buildBtn').disabled = !canActOnLand; $('endBtn').disabled = !isMyTurn(); $('logPanel').innerHTML=(state.logs||[]).map(x=>`<div class="log">${x}</div>`).join(''); }
function renderPlayers(){ $('playersPanel').innerHTML = (state.turnOrder||[]).map(id=>{ const p=state.players[id]; return `<div class="player ${id===currentId()?'active':''}"><div class="avatar">${p.avatar}</div><b>${p.name}${p.isAI?' AI':''}</b><div class="money">$${p.money}</div><small>位置 ${p.pos}｜總部 ${p.homeIndex ?? 0}</small></div>` }).join(''); }
